from authlib import new_combined_secret, get_new_secret, hash256, bytes_to_string, verify_code
...
def signup (email, pin, devid, code, user_secret):
    if (verify_code(code, email, devid) == True):

        user_secret = get_new_secret()
        server_secret = get_new_secret()
        combined_secret = new_combined_secret(user_secret, server_secret)
